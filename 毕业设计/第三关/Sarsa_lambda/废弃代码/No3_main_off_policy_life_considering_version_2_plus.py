...
#基于version_2:增加了reload——training模块，保存下来的训练可以重新加载并在此基础上继续
#以一个恒定的epsilon值训练
#貌似这种训练起到的效果并不好
...
class sarsa_agent(object):
    ...
    def learning(self,pro_fine,lambda_,gamma,alpha,max_episode_num,Q=None,epsilon=None):
        if Q is not None:
            self.Q=Q
            self.E_copy_Q()
        else:
        ...
        for num_episode in range(max_episode_num):
            ...
            if present_percent_0!=present_percent_1:
                ...
                if present_percent_0 % 1==0:
                    time_check=time.time()-time_start
                    hs=int(time_check/3600)
                    ms=int((time_check % 3600)/60)
                    ss=time_check % 60
                    print("运行时间：{}小时，{}分钟，{:.2f}秒".format(hs,ms,ss))
            ...
            for i in range(10):
                ...
                if epsilon is not None:
                    pos_1=self.new_policy(str(state_0),epsilon) #这里是行动策略，epsilon-greedy
                else:
                ...
                if epsilon is not None:
                    pos_2=self.new_policy(str(state_1),epsilon) #目标更新，greedy
                else:
    ...
    def reload_training(self,Q_np_name,times):
        #这里我们尝试以固定概率 继续训练
        with open(Q_np_name, 'rb') as file:
            Q=pickle.load(file)
        #继续训练时参数尽量保持一致
        out=self.learning(pro_fine=0.6,
                       lambda_=0.01,
                       gamma=0.8,
                       alpha=0.01,              #更改一下学习率看看效果
                       max_episode_num=times,
                       Q=Q,
                       epsilon=0.1)
        return out
...
'''
file_Q='Random_life_saving_100w_version_2.pickle'
a.reload_training(file_Q, 10000)
a.save_as('Random_life_saving_100w+1w_version_2.pickle')
...
Q=a.learning(pro_fine=0.6,
           ...
           gamma=0.8,
           alpha=0.1,
           max_episode_num=2000000,
           Q=None,
           epsilon=None)

file = open('Random_life_saving_200w_version_2_plus.pickle', 'wb')
pickle.dump(Q, file)
file.close()
'''
a.reload('Random_life_saving_100w.pickle')
