...
非常明显的一点：策略一点点的改变就会影响最终的结果

也就是说：虽然考虑的是state+动作，但是实际上小偷的动作的分布是被考虑进去的
这个动作分布主要体现再 下一个状态
训练时下一个状态的选择是随机的 而且是被记录的
但是训练小偷以后这个随机策略就被改变了 动作分布不同策略不收敛
"""
import numpy as np
import random
import pickle
...
class chess_agent(object):
    ...
    def policy_police(self,state_name,epsilon):
        #警察的策略，必须是要基于训练好的警察策略才行
        Q_s=self.Q_police[state_name]
    ...
    def policy_thief(self,state_name,epsilon):
    ...
    def learning_thief(self,lambda_,gamma,alpha,file_name,epoch_step=30,max_episode_num=100000,eval_iteration=10000):
        with open(file_name, 'rb') as file:
            self.Q_police=pickle.load(file)
            #print(self.Q_police)
        self.Q={}
        ...
        for num_episode in range(max_episode_num):
            if (num_episode % eval_iteration == 0 and num_episode!=0) or num_episode==max_episode_num-1:
                ...
                end='w_thief_model.pt'
                file_name=head+mid+end
                file = open(file_name, 'wb')
                pickle.dump(self.Q, file)
                file.close()
                success_rate,average_reward=self.evaluating_thief(1000,epoch_step)
            ...
            for i in range(epoch_step):
                state_0=str(self.pos_all)
                #action_thief=self.policy_thief(state_0, 1) #与警察略有不同 因为小偷要先跑 所以只有三者的位置
                action_thief=self.policy_thief(state_0, epsilon)
                state_for_police=self.get_state(action_thief)
                self.thief_act(action_thief)
                
                union_action=self.policy_police(state_for_police, 0) 
                pos_all,reward,is_done=self.union_act(union_action)
                
                #reward修正 因为小偷与警察看作0和博弈 必须有一方受伤
                reward=-reward
                
                state_1=str(pos_all)
                _action_thief=self.policy_thief(state_1, 0)
                
                
                Q_error=gamma*self._get_(self.Q, state_1, _action_thief)-self._get_(self.Q, state_0, action_thief)
                theta=reward+Q_error if not is_done else reward
                E_value=self._get_(self.E_epoch, state_0, action_thief)
                self._set_(self.E_epoch, state_0, action_thief, E_value+1)
    ...
    def evaluating_thief(self,times,epoch_step):
        #也需要针对小偷做相应的修改
        print("评估开始")
        tol_success=0
        tol_reward=0
        for num in range(times):
            ...
            for i in range(epoch_step):
                state_0=str(self.pos_all)
                action_thief=self.policy_thief(state_0, 0)
                state_police=self.get_state(action_thief)
                self.thief_act(action_thief) #已经收集动作，小偷可以行动
                union_action=self.policy_police(state_police,0)
                pos_all,reward,is_done=self.union_act(union_action)
                reward=-reward
                tol_reward+=reward
                if is_done:
...
eval_iteration=5000
epoch_step=30
Q,eval_sr,eval_ar=a.learning_thief(lambda_=0.1, gamma=0.8, alpha=0.01,file_name=file_name,epoch_step=epoch_step, max_episode_num=training_times,eval_iteration=eval_iteration)
