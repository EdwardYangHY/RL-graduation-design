...
class DQN(object):
    ...
    def new_learning(self):
        ...
        loss=self.loss_func(q_eval*IS_weights,q_target*IS_weights) # 重要性采样偏置修正
        #loss=self.loss_func(q_eval,q_target) # 没有重要性采样偏置修正
        #max_loss=max(loss)
...
Q=b.training(max_episode_num=20000)
y=np.array(Q)
x=np.arange(0,len(y),1)
plt.plot(x, y)
plt.show()
b.save_model("DQN_PER_2W_alpha_10_6.pt")
